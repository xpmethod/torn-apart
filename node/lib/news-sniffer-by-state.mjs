import { statSync, existsSync, mkdirSync, writeFile, readFile, readFileSync } from "fs";
    ...
    parse(data, { columns: true }, (err, facilities) => {
      ...
      facilities.map( facility => {
        ...
        if (facility["State"]){
          ...
          let doSearch = true;
          // check to see if we've already searched
          if (existsSync(thePath)){
            // This is from the morning, a few hours before the first run of the script.
            const beforeNew = 1531130591000;
            const timeDiff = beforeNew - statSync(thePath).mtimeMs;
            if (timeDiff < 0){
              // We updated the report.
              doSearch = false;
            }
          }
          if (doSearch === false) {
