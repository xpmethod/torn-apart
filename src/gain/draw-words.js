...
export default function(words) {
  ...
  select("#wordcloud-g")
    ...
    .style("font-size", function(d) {
      return Math.ceil(Math.pow(d.total_value, 0.26)) / 4 + 4 + "px";
    })
    .style("fill", function(d) {
      return colour[d.years - 1];
    })
    ...
    .each(d => {
      ...
      if (d.name === "CCA") {
        ...
        <strong>${format("$,.0f")(
          Math.ceil(d.total_value)
        )}</strong> for <strong>${d.product}</strong> over <strong>${
          d.years
        }</strong> ${d.years > 1 ? "years" : "year"}`;
      } else {
        d.tooltip = `${d.name}:
        <strong>${format("$,.0f")(
          Math.ceil(d.total_value)
