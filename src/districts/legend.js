...
export default function(bins) {
  ...
  $("#legend")
    ...
    .html(() => {
      const html = [
        "<div class='px-3 py-2'><div><p data-i18n='ta-ice-money-since-2013'></p></div><div class='d-flex'>"
      ...
      _.each(breaks, (bk, i) => {
        ...
        <svg class="legend-svg" height="${3 * height}" width="20">
          <rect height="${height}" width="20" class="legend-polygon" fill="${purple}" opacity="${
          opacityRange5[i]
        }"/>
          <rect height="${height}" width="20" class="legend-polygon" fill="${green}" opacity="${
          opacityRange5[i]
        }" transform="translate(0,${height})"/>
          <rect height="${height}" width="20" class="legend-polygon" fill="${orange}" opacity="${
          opacityRange5[i]
