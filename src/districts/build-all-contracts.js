    ...
    parse(data, { columns: true }, (err, awards) => {
      ...
      stringify(
        awards.map(award => {
          ...
          return {
            ...
            companyCongCode: convertToCongCode(
              ...
              award.recipient_congressional_district
            ),
            performanceCongCode: convertToCongCode(
              award.primary_place_of_performance_state_code,
              award.primary_place_of_performance_congressional_district
            )
          };
        }),
        {
        ...
        (err, output) => {
          if (err) throw err;
          writeFile(
            path.join("data", "districts", "thin_contracts.csv"),
            output,
            err => {
              if (err) throw err;
