...
export default function(decorations) {
  readFile(path.join("data", "follow_the_money_data.csv"), (err, data) => {
    ...
    parse(data, { columns: true }, (err, rawAwards) => {
      ...
      _.each(awards, award => {
        award.naics_cat = _.find(product_taxonomy, {
      ...
      _.each(
        ...
        sources => {
          _.each(sources.source_array, source => {
            ...
            const name = origName.match(/^\d*$/)
              ? _.find(decorations, { duns: origName }).cleanName
              : origName;
            graph.nodes.push({
              id: source,
              name,
      ...
      _(graph.nodes.filter(node => node.category === "company")).each(
        company => {
          company.awards = awards
            .filter(
              award => award.recipient_duns === company.id.replace(/\|.*$/, "")
            )
            .map(thinAward);
      ...
      _(graph.nodes)
        ...
        .each(companyNode => {
          _(graph.links)
            ...
            .each(companyLink => {
              companyLink.contract_value = _.reduce(
                awards.filter(
                  award =>
                    award.recipient_duns === companyNode.id &&
                    award.product_combo === companyLink.target
                ),
                (sum, award) => {
                  return sum + _.toInteger(award.current_total_value_of_award);
                },
          ...
          _(
            graph.nodes.filter(node => node.category === "product category")
          ).each(productCategory => {
            productCategory.awards = awards
              .filter(award => award.naics_cat === productCategory.name)
              .map(thinAward);
            productCategory.total_value = productCategory.awards.reduce(
              (sum, award) => {
                return sum + _.toInteger(award.current_total_value_of_award);
              },
              0
            ...
            .each(productCategoryNode => {
              _(graph.links)
                ...
                .each(productCategoryLink => {
                  productCategoryLink.contract_Value = _.reduce(
                    awards.filter(
                      award =>
                        award.naics_cat === productCategoryNode.name &&
                        award.product_combo === productCategoryLink.target
                    ),
                    (sum, award) => {
                      return (
                        sum + _.toInteger(award.current_total_value_of_award)
                      );
                    },
                    0
                  );
      ...
      writeFile(
        path.join("data", "freezer", "graph.json"),
      ...
      function thinAward(award) {
        return {
          ...
          award_description: titleUp(
            award.award_description.replace(/^IGF\S*/, "")
          ),
          recipient_city: award.recipient_city_name
            .split(" ")
            .map(word => _.capitalize(word))
            .join(" "),
          recipient_state: award.recipient_state_code,
          // recipient_duns: award.recipient_duns,
          recipient_name: _.find(decorations, { duns: award.recipient_duns })
            .cleanName,
