...
export default function(d){
  const awards = d.awards.sort((a, b) => b.current_total_value - a.current_total_value);
  const html = ["<div class='spreadsheet'><table class='table table-hover table-sm'>"];
  html.push("<thead><tr class='thead-dark'>");
  _.each(["ta-v2-dba", "ta-v2-location", "ta-v2-category", "ta-v2-product-service", "ta-v2-current-value"], header => {
    let th = "<th>";
    if(d.color){
  ...
  _.each(awards, award => {
    ...
    html.push(`<td><small>$${bigMoneyFormat(award.current_total_value_of_award)}</small></td>`);
    html.push("</tr><tr>");
    html.push("<td>&nbsp;</td>");
