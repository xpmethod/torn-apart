  ...
  if ($("#tree-sidebar").text().length < 1) {
    ...
    if (window.matchMedia("(max-width: 576px)").matches) {
      $("#tree-sidebar").prepend(
        `<h3 id='click-on-cube' data-i18n='ta-click-on-cube-to-see-contractor'>${$.i18n(
          "ta-click-on-cube-to-see-contractor"
        )}</h3>`
      );
    } else {
      const firstCompany = $(
      ...
      treeSelectCell(
        firstCompany.attr("id").replace("treemap-card-", "parent-")
      );
