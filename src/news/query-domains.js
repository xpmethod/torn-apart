...
export default function(apiKey) {
  ...
  const indexed = createWriteStream(join(workDir, "indexed.txt"), {
    flags: "a"
  });
  const notIndexed = createWriteStream(join(workDir, "not_indexed.txt"), {
    flags: "a"
  });
  ...
  if (existsSync(join(workDir, "nos.txt"))) {
    ...
    const ss = JSON.parse(
      readFileSync(resolve("data", "news-sources-by-state.json"))
    );
          ...
          chunk.map(domain => {
            if (
              response.articles.filter(article =>
                article.url.match(domain.replace(/^www\./, ""))
              ).length > 0
            ) {
