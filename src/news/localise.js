...
fs.readFile("data/news-sources-by-state.json", "utf8", function(err, data) {
  ...
  var obj;
  fs.readFile(
    "data/news-sniffer-reports/TELLECO/Colorado.json",
    "utf8",
    function(err, data) {
      //going to need to do this for every file in the folder tree, and in my case they will be called "everything.json"
      if (err) throw err;
      obj = JSON.parse(data);
      ...
      for (item in resultsArray) {
        results =
          ...
          resultsArray[item][0] +
          ", " +
          JSON.stringify(resultsArray[item][1]);
      }
