...
export default function(strName){
  ...
  // it's in ALL CAPS.
    ...
    if(foundTripleCons !==-1){
      if(strName.substr(foundTripleCons, 2)!=="Mc" && !strName.substr(foundTripleCons, 2).match(/th/i)===true && !strName.substr(foundTripleCons, 1).match(/[sS]/)===true){ 
        strName = strName.replace(/\b(?=[a-z]{3})[^aeiouy]{3}.*?\b/i, function(match){return match.toUpperCase();});
      }
