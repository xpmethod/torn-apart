...
export default function(map) {
  ...
  const columns = Math.floor(vizWidth / (128 + 6 + 0.5 * rem));
  const rows = Math.floor(vizHeight / (128 + 6 + 0.5 * rem));
  const images = shuffle(imgurImages)
  ...
  for (let i = 1; i <= rows; i++) {
    ...
    matrix =
      ...
      images
        .splice(0, columns)
        .map(
          image => `<div class="eye-tile-div m-1" 
      data-lat="${image.lat}" data-lon="${image.lon}" >
      <img class="eye-img rounded" src="${image.thumb}"></div>`
        )
        .join("");
    matrix = matrix + "</div>";
  }
  ...
  $(".eye-tile-div").click(function() {
    ...
    const pixelCoords = map.project(
      L.latLng($(this).data("lat"), $(this).data("lon")),
      15
    );
    const imgfromOrigin = [
      $(this).position().left + 32 - $(window).width() / 2,
      $(window).height() / 2 - $(this).position().top - 128
    ];
    const newCenter = L.point(
      pixelCoords.x - imgfromOrigin[0],
      pixelCoords.y + imgfromOrigin[1]
    );
    const newlatlng = map.unproject(newCenter, 15);
    $(this)
      .removeClass("eye-tile-div")
      .css("transform", "none");
    $(".eye-tile-div")
      .removeClass("shadow")
      .css("transform", "scale(0.25, 0.25)")
      .css("transform-origin", "50% 50%")
      .children("img")
      .css("border-width", "12px");
    $(this)
      .addClass("eye-tile-div")
      .addClass("shadow");
