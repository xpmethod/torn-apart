...
export default function(chart, svgHeight, svgWidth) {
  ...
  $(chart.id.replace("svg", "no")).html(
    format(",.0f")(chart.data[chart.data.length - 1][1])
  );
  ...
  if (chart.showFY) {
    chart.svg
      ...
      .attr(
        "transform",
        `translate(${svgWidth - chart.margins.right},${svgHeight - 4})`
      )
      // Add some text.
      .attr("text-anchor", "end")
      .attr("data-i18n", "ta-v1-fiscal-year-begins");
  }
}
