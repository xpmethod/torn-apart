...
export default function(operators) {
  ...
  ["GEO", "CCA", "ORR"].forEach(code => {
    ...
    operators.data.push({
      ...
      adpCount: sum(
        Data.filter(f => f["Facility.Operator"] === code).map(
          d => +d["FY18.ADP"]
        )
      )
    });
  ...
  [
    ...
    operators.data[0].facilityCount += Data.filter(
      f => f["Facility.Operator"] === code
    ).length;
    operators.data[0].adpCount += sum(
      Data.filter(f => f["Facility.Operator"] === code).map(d => +d["FY18.ADP"])
    );
  });
  // Now fill in facilityCount and adpCount for the Government object in the data array.
  operators.data[1].facilityCount =
    Data.filter(d => +d["FY18.ADP"] > 0).length -
    sum(operators.data.map(d => +d.facilityCount));
  operators.data[1].adpCount =
    sum(Data.map(d => +d["FY18.ADP"])) -
    sum(operators.data.map(d => +d.facilityCount));
  // Finally, associate a color with each of the five data objects in operators.data.
  [fadedpink, fadedgreen, fadedorange, fadedpurple, fadedblack].forEach(
    (color, i) => {
