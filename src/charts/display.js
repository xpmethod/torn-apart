...
export default function(map) {
  ...
  const tp = {
    ...
    id: "#total-places-svg",
    ymax: 400,
    i18n: "ta-v1-number-of-facilities",
    showFY: true
  };
  const adp = {
    data: [],
  ...
  const operators = {
    ...
    id: "#operators-svg",
    number: "facilityCount",
    svgWidth: thirdWidth
  };
  const operatorsAdp = {
    margins: { top: 0, bottom: 0, left: 0, right: 0 },
    id: "#operators-adp-svg",
    number: "adpCount",
    svgWidth: thirdWidth
  };
  ...
  const mandays = {
    data: Data.map(row => {
      ...
      data-latlng="${row.lat},${row.lon}" data-detloc="${
        row.DETLOC
      ...
      const operator = facOperators.filter(
        o => o.code === row["Facility.Operator"]
      )[0];
  ...
  [tp, adp, bookins, operators, operatorsAdp].forEach(chart =>
    initChart(chart, svgWidth, svgHeight)
  );

  // build the bar charts.
  [tp, adp, bookins].forEach(dataObject =>
    buildBarChart(dataObject, svgHeight, svgWidth)
  );
