...
export default function(map) {
  ...
  data.map(datum => {
    ...
    const simulation = forceSimulation()
      .force(
        "collide",
        forceCollide(d => max([6, Math.pow(d.r, 0.62)])).iterations(16)
      )
    ...
    simulation.nodes(datum.blacksites).on("tick", ticked);
  });
}
...
function fleeMouse() {
  select(this).attr(
    "transform",
    `translate(${randomNormal(0, 25)()},${randomNormal(0, 25)()})`
  );
}
